...
class TestValidatorRulesUnit(unittest.TestCase):
    ...
    def test_if_is_a_dataclass(self):
        self.assertTrue(is_dataclass(
            ValidatorRules(value="gabriel", prop="name")))
    ...
    def test_required_rule_single_error(self):
        ...
        for data in invalid_datas:
            ...
            with self.assertRaises(ValidationException, msg=error_log_msg) as assert_error:
                value = data["value"] if data["value"] is None else str(
                    data["value"]).strip()

                self.assertIsInstance(
                    ValidatorRules.values(
                        value, data["prop"], throw_error=True).required(),
                    ValidatorRules
                )
    ...
    def test_string_rule_single_error(self):
        ...
        for data in invalid_datas:
            ...
            with self.assertRaises(ValidationException, msg=error_log_msg) as assert_error:
                self.assertIsInstance(
                    ValidatorRules.values(
                        data["value"], data["prop"], throw_error=True).string(),
                    ValidatorRules
                )
    ...
    def test_max_length_rule_single_error(self):
        ...
        for data in invalid_datas:
            ...
            with self.assertRaises(ValidationException, msg=error_log_msg) as assert_error:
                self.assertIsInstance(
                    ValidatorRules.values(
                        data["value"], data["prop"], throw_error=True).max_length(5),
                    ValidatorRules
                )
    ...
    def test_boolean_rule_single_error(self):
        ...
        for data in invalid_datas:
            ...
            with self.assertRaises(ValidationException, msg=error_log_msg) as assert_error:
                self.assertIsInstance(
                    ValidatorRules.values(
                        data["value"], data["prop"], throw_error=True).boolean(),
                    ValidatorRules
                )
    ...
    def test_composed_rules(self):
        ...
        with self.assertRaises(ValidationException) as assert_error:
            self.assertIsInstance(
                ValidatorRules.values(
                    6, "prop", throw_error=True).required().boolean(),
                ValidatorRules
            )
