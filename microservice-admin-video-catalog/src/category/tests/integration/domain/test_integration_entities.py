...
class TestCategoryIntegration(unittest.TestCase):
    ...
    def test_invalid_name_cases(self):
        ...
        for i in invalid_datas:
            ...
            self.assertEqual(assert_error.exception.args[0], 'Entity Validation Error') #NOSONAR
            self.assertEqual(assert_error.exception.error, i['error_msg']) #NOSONAR
    ...
    def test_invalid_is_active_cases(self):
        ...
        self.assertEqual(assert_error.exception.error, 'is_active:The is_active must be boolean;')
    ...
    def test_update_cases(self):
        ...
        self.assertEqual(assert_error.exception.error, 'name:The name is required;')

        with self.assertRaises(EntityValidationException) as assert_error:
            category.update(123, '') #NOSONAR
        self.assertEqual(assert_error.exception.error, 'name:The name must be a string;')
